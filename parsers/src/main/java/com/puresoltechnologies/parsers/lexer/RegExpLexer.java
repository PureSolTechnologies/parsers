...
public class RegExpLexer implements Lexer {
	...
	while (text.length() > 0) {
		...
		throw new LexerException("No token found for '" + exceptionText + "' in line "
			+ sourceCodeLine.getSource().getHumanReadableLocationString() + ":"
			+ sourceCodeLine.getLineNumber() + ".");
	    }
	    if (logger.isTraceEnabled()) {
		logger.trace("Found token: " + token + " / " + token.getMetaData());
	    }
    ...
    private Token findNextToken(StringBuffer text, StringWithLocation fullText, int position) {
	Token nextToken = null;
	for (TokenDefinition definition : grammar.getTokenDefinitions().getDefinitions()) {
	    Matcher matcher = definition.getPattern().matcher(text);
	    if (!matcher.find()) {
		continue;
	    }
	    String tokenText = matcher.group(0);
	    if ((nextToken == null) || (tokenText.length() > nextToken.getText().length())) {
		SourceCodeLine sourceCodeLine = fullText.getSource(position);
		TokenMetaData metaData = new TokenMetaData(sourceCodeLine.getSource(), sourceCodeLine.getLineNumber(),
			StringUtils.countLineBreaks(tokenText) + 1, fullText.getColumn(position));
		nextToken = new Token(definition.getName(), tokenText, definition.getVisibility(), metaData);
	    }
	}
	return nextToken;
    }
	...
	try {
	    RegExpLexer cloned = (RegExpLexer) super.clone();
	    ReflectionUtils.setField(cloned, "grammar", grammar);
		...
		SourceCode clonedSourceCode = new SourceCode(sourceCode.getName(), sourceCode.getInternalLocation());
