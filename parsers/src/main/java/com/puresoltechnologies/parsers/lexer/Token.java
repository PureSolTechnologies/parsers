	...
	public Token(String name, String text, Visibility visibility,
		...
		if ((name == null) || (name.isEmpty())) {
			throw new IllegalArgumentException(
					"Name must not be null or empty for token.");
		}
		this.name = name;
		if ((text == null) || (text.isEmpty())) {
			throw new IllegalArgumentException(
					"Text must not be null or empty for token.");
		}
		this.text = text;
		if (visibility == null) {
			throw new IllegalArgumentException(
					"Visibility must not be null for token.");
		}
		this.visibility = visibility;
		if (metaData == null) {
			throw new IllegalArgumentException(
					"MetaData must not be null for token.");
		}
		this.metaData = metaData;
		hashcode = Objects.hash(name, text, visibility, metaData);
	}
