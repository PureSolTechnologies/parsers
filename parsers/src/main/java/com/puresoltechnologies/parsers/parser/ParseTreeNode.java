...
public class ParseTreeNode implements TreeNode<ParseTreeNode>, Serializable, Cloneable {
    ...
    /**
     * This flag specifies whether the node is allowed to be a node or not. If not,
     * the children will be assigned to its parent on the location of itself and the
     * node is deleted during tree normalization.
     */
    private final boolean node;
    /**
     * This flag specified whether a stacking of this node type is allowed or not.
     * If stacking is allowed, than nodes of this type can contain each other.
     * Otherwise the nodes would be reduced to one occurrence and all children would
     * be assigned to one single node during tree normalization.
     */
    ...
    public ParseTreeNode(String name, Token token, boolean node, boolean stackingAllowed) {
	this.name = name;
	this.token = token;
	this.node = node;
    ...
    public void addChildrenInFront(List<ParseTreeNode> children) throws TreeException {
     ...
     * normalization or not. Have a look to {@link #node} for more details on this
     * subject.
     * 
    ...
    /**
     * This method returns whether this node is allowed to be stacked or not. See
     * {@link #stackingAllowed} for details.
     * 
    ...
    private void getSubTrees(ParseTreeNode branch, List<ParseTreeNode> subTrees, String name) {
    ...
    private void fillBuffer(StringBuffer buffer, ParseTreeNode parserTree, int depth) {
	...
	try {
	    ParseTreeNode cloned = (ParseTreeNode) super.clone();
	    ReflectionUtils.setField(cloned, "name", name);
	    if (token != null) {
		ReflectionUtils.setField(cloned, "token", token.clone());
	    }
	    ReflectionUtils.setField(cloned, "node", node);
	    ReflectionUtils.setField(cloned, "stackingAllowed", stackingAllowed);
	    cloned.parent = parent;
	    if (metaData != null) {
		cloned.metaData = metaData.clone();
	    }
	    cloned.children.addAll((List<ParseTreeNode>) children.clone());
	    return cloned;
