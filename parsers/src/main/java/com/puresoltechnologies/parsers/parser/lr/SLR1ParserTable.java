...
public class SLR1ParserTable extends AbstractParserTable {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SLR1ParserTable.class);
	...
	for (int state = 0; state < itemSetCollection.getStateNumber(); state++) {
		...
		if (lr0Item.hasNext()) {
		...
		} else if (lr0Item.getProduction().equals(getGrammar().getProductions().get(0))) {
		    ...
		    for (Construction lookahead : follow.get(new NonTerminal(lr0Item.getProduction().getName()))) {
			addAction(state, lookahead,
				new ParserAction(ActionType.REDUCE, lr0Item.getProduction().getId()));
		    }
		}
	    }
	}
	...
	FileUtilities.writeFile(directory, new File("Follow"), follow.toString());
